...
from model_doubleUnknow import AnomalyDetectionModel
...
def main():
    ...
    PULL_NORM_FPR = 0.03  # 只允许 1% 的“真实正常”被我们从 Normal 拽走（控制二分类不崩）
    ...
    # ------------ 2. KMS+PCA 预处理：缺失填充 -> 标准化 -> PCA ------------
    print("🔧 数据预处理（KMS+PCA：缺失填充→标准化→PCA）...")
    # 先填充缺失，避免 scaler / PCA 遇到 NaN
    train_df = train_df.ffill().bfill()
    test_df = test_df.ffill().bfill()

    scaler = StandardScaler()
    Xtr = scaler.fit_transform(train_df[feature_cols].astype(np.float32))
    Xte = scaler.transform(test_df[feature_cols].astype(np.float32))

    n_comp = int(config.get("pca_n_components", 0.99))
    pca = PCA(n_components=n_comp, random_state=int(config.get("pca_random_state", 42)))
    ...
    # 写回 DataFrame：后续窗口构造直接用 pca_cols
    for i, c in enumerate(pca_cols):
        train_df[c] = Xtr_p[:, i]
        test_df[c] = Xte_p[:, i]

    # 只保留必要列，避免后面误用原始特征
    train_df = train_df[[ID_COL, TIME_COL, LABEL_COL] + pca_cols]
    test_df = test_df[[ID_COL, TIME_COL, LABEL_COL] + pca_cols]

    feature_cols = pca_cols
    print(f"[PCA] {len(pca_cols)} -> {len(pca_cols)} | explained_var={explained:.4f}")
    ...
    # ------------ 4.1 KMS：对训练窗口做 KMeans-SMOTE（只处理“无 padding”的完整窗口）------------
    if config.get("use_kms_pca", False):
        W = config["window"]
        D = X_tr.shape[2]

        full_idx = ~M_tr.any(axis=1)  # True 表示该窗口没有 padding
        X_full = X_tr[full_idx].reshape(-1, W * D)
        y_full = y_tr[full_idx]

        cnt_full = dict((int(c), int(v)) for c, v in Counter(y_full.tolist()).items())
        print("[KMS] full-window counts before KMeansSMOTE:", cnt_full)

        if X_full.shape[0] > 0 and len(cnt_full) > 1:
            # k_neighbors 不能超过最小类样本数-1，否则会报错
            min_cnt = min(cnt_full.values())
            k_cfg = int(config.get("kms_k_neighbors", 3))
            k = min(k_cfg, max(1, min_cnt - 1))
            if k < 1:
                print("[KMS] min class too small, skip KMeansSMOTE.")
            else:
                n_clusters = int(config.get("kms_n_clusters", 32))  # 32/64 都行
                cluster_balance_threshold = float(config.get("kms_cluster_balance_threshold", 0.01))  # 0.0~0.02
                kms_batch_size = int(config.get("kms_batch_size", 2560))  # >=2560 避免 warning

                kmeans_est = MiniBatchKMeans(
                    n_clusters=n_clusters,
                    random_state=int(config.get("kms_random_state", 42)),
                    batch_size=kms_batch_size,
                )

                kms = KMeansSMOTE(
                    random_state=int(config.get("kms_random_state", 42)),
                    k_neighbors=k,
                    kmeans_estimator=kmeans_est,
                    cluster_balance_threshold=cluster_balance_threshold,
                )

                try:
                    X_res, y_res = kms.fit_resample(X_full, y_full)
                except RuntimeError as e:
                    print(f"[KMS] RuntimeError: {e}")
                    print("[KMS] fallback -> plain SMOTE (keeps KMS+PCA pipeline runnable)")
                    sm = SMOTE(random_state=int(config.get("kms_random_state", 42)), k_neighbors=k)
                    X_res, y_res = sm.fit_resample(X_full, y_full)

                X_res = X_res.reshape(-1, W, D)
                M_res = np.zeros((X_res.shape[0], W), dtype=bool)

                # 把“有 padding 的窗口”原样拼回去（避免 KMS 生成 padding）
                X_pad, y_pad, M_pad = X_tr[~full_idx], y_tr[~full_idx], M_tr[~full_idx]

                X_tr = np.concatenate([X_res, X_pad], axis=0)
                y_tr = np.concatenate([y_res, y_pad], axis=0)
                M_tr = np.concatenate([M_res, M_pad], axis=0)

                # 打乱（更像正常训练）
                perm = np.random.permutation(len(y_tr))
                X_tr, y_tr, M_tr = X_tr[perm], y_tr[perm], M_tr[perm]

                print("[KMS] window counts after KMeansSMOTE:", dict(Counter(y_tr.tolist())))
        else:
            print("[KMS] not enough full windows or classes, skip KMeansSMOTE.")
